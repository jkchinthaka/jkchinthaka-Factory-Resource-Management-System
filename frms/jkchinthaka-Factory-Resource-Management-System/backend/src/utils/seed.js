...
async function seed() {
  try {
    ...
    const masterConfig = {
      ...
      password: process.env.DB_PASSWORD || 'Nelna@123',
      database: 'master',
      options: {
    ...
    // Step 3: Run schema (split on semicolons; all statements have IF NOT EXISTS guards)
    ...
    for (const stmt of statements) {
      if (stmt.trim()) {
        try {
          ...
          logger.warn(`Schema statement warning: ${e.message}`);
        }
      }
    }
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        await db.query(
          'INSERT INTO water_meter_data (date, intake, [PPU 1 Reading], [FPU 1 Reading], [Chiller Reading], [Cooling tower Reading], cost, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, 1)',
...
async function seed() {
  try {
    ...
    // Connect to the database (MySQL; schema is pre-created by Docker init)
    const db = require('../models/db');
